 ...
 */
public class ArrayMathPerformance {
            ...
            for (int x = 0; x < image.getWidth(); ++x) {
                for (int y = 0; y < image.getHeight(); ++y) {
                    for (int channel = 0; channel < image.getNumOfChannels(); ++channel) {
                        image.set(x, y, channel, mult * image.get(x, y, channel));
                    }
                }
            }
        }
            ...
            for (int x = 0; x < array.getWidth(); ++x) {
                for (int y = 0; y < array.getHeight(); ++y) {
                    for (int channel = 0; channel < array.getNumOfChannels(); ++channel) {
                        array.setUnsafe(x, y, channel, mult * array.getUnsafe(x, y, channel));
                    }
                }
            }
        }
    ...
    private static double testByteArray(final int width, final int height, final int numOfIterations) {
        ...
        for (int num = 0; num < numOfIterations; ++num) {
            for (int i = 0; i < array.length; ++i) {
                array[i] = (byte) (mult * (array[i] & 0xFF));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int mSize : sizes) {
            ...
            System.out.println("    Image8I:       " + testImage8I(mSize, mSize, numOfIterations));         // ~7
            System.out.println("    ImageArray64F: " + testImageArray64F(mSize, mSize, numOfIterations));   // ~1
            System.out.println("    ImageArray8I:  " + testImageArray8I(mSize, mSize, numOfIterations));    // ~5
            System.out.println("    double[]:      " + testDoubleArray(mSize, mSize, numOfIterations));     // =1
            System.out.println("    byte[]:        " + testByteArray(mSize, mSize, numOfIterations));       // ~2
        }
    }
}
