...
public class VideoFileReader implements VideoReader, Runnable {
    ...
    public void open() throws IOException {
        ...
        if (this.isOpen()) {
            ...
            // Generate an error!
            if (this.bufImg == null) {
                this.generateError("Can not closed video file that was opened!");
            }
        }
        ...
        if (this.bufImg == null && this.isOpen()) {
            this.generateError("Can not open video file!");
        }
    ...
    public void run() {
        try {
            ...
            if (videoStreamId == -1) {
                throw new RuntimeException("Could not find video stream in container: " + this.filePath);
            }
            // Try to open up our decoder so it can do work.
            if (this.videoCoder.open() < 0) {
                throw new RuntimeException("Could not open video decoder for container: " + this.filePath);
            }
            ...
            if (this.videoCoder.getPixelType() != IPixelFormat.Type.BGR24) {
                ...
                if (resampler == null) {
                    throw new RuntimeException("Could not create color space resampler for: " + this.filePath);
                }
            }
            ...
            while (this.container.readNextPacket(packet) >= 0 && this.isOpen) {
                ...
                if (packet.getStreamIndex() == videoStreamId) {
                    ...
                    while (offset < packet.getSize()) {
                        ...
                        if (bytesDecoded < 0) {
                            throw new RuntimeException("Got error decoding video in: " + this.filePath);
                        }
                    ...
                    if (picture.isComplete()) {
                        ...
                        if (resampler != null) {
                            ...
                            if (resampler.resample(newPic, picture) < 0) {
                                throw new RuntimeException("Could not resample video from: " + this.filePath);
                            }
                        }
                        ...
                        if (newPic.getPixelType() != IPixelFormat.Type.BGR24) {
                            throw new RuntimeException("Could not decode video as BGR 24 bit data in: " + this.filePath);
                        }
                        ...
                        if (firstTimestampInStream == Global.NO_PTS) {
                            ...
                            // In milliseconds (10^{-3} seconds).
                            final long mSecToSleep = (mSecStreamTimeSinceStartOfVideo - (mSecClockTimeSinceStartOfVideo + mSecTolerance));
                            if (mSecToSleep > 0) {
                                try {
